...
teacher_config_file = "ppo_aerial_quad.yaml"
...
if __name__ == "__main__":
    ...
    print("Loading teacher config: ", teacher_config_file)
    with open(teacher_config_file, "r") as stream:
        teacher_config = yaml.safe_load(stream)

        teacher_config = update_config(teacher_config, args)

        try:
            print("Loading teacher config...", teacher_config)
        except yaml.YAMLError as exc:
            print(exc)
        teacher_config["params"]["network"]['name'] = "batched" #"dectestnet"
    ...
    loaded_configs["dt"] = 0.005 # set dt here
    loaded_configs["max_episode_steps"] = 2*600 # set max episode length here
    loaded_configs["randomize_descriptor"] = True if args['dr'] else None # whether to randomize the descriptor of the robot during training (for domain randomization)
    loaded_configs["train_individual"] = False # whether to train individual robots or a single policy for all robots
    ...
    config["params"]["teacher_params"] = teacher_config
    print("Final config used for training: ", config["params"].keys())
    runner.load(config)
    runner.run(args)
